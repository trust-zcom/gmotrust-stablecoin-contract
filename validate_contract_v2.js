...
async function Validate() {
  console.log("Connecting to Token_v2 contract...");
  const token = await TokenV2.deployed();
  await TokenV2.at(token.address);
  console.log("Token_v2 found.");

  console.log("\n");

  const zusd = await ZUSD.deployed();
  let proxiedToken = await TokenV2.at(zusd.address);
  console.log("****** ZUSD ******");
  ...
  proxiedToken = await TokenV2.at(gyen.address);
  console.log("****** GYEN ******");
